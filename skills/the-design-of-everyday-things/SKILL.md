...
**Affordances vs Signifiers** — affordances are possible actions; signifiers are the cues that
make them discoverable. *Procedure:* for each intended action, name its affordance, then make
sure a visible signifier points to it; if a user/agent can't perceive the action, add a
signifier. *Use when* "they didn't know they could do X" or a control is invisible.

**The Seven Stages of Action** — goal → plan (intention) → specify → perform → perceive →
interpret → compare. *Procedure:* walk a real task through all seven stages and find the stage
where it breaks. *Use when* you need to locate *exactly where* an interaction fails.
