...
1. **Reframe as a bet.** State the decision as a wager on an uncertain future; name the options.
2. **Strip out resulting.** If reviewing, judge the *process* given what was knowable, not the result.
3. **Put odds on it.** Estimate probabilities and payoffs; say how sure you are ("70%"), and where you're "not sure."
4. **Time-travel.** Run 10-10-10, backcast the success, and premortem the failure.
5. **Pod it.** Run high-stakes calls past a truthseeking group under CUDOS; actively invite dissent.
6. **Decide and log the bet.** Record the decision, the confidence, and the reasoning — so later you can grade the process, not the outcome.
7. **Field the outcome.** When results arrive, sort luck vs. skill; update the process only on repeatable signal.
