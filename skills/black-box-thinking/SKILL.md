...
*Procedure:* break the goal into components; for each, run a controlled comparison (A/B test or
RCT where possible); keep only changes the data validates; stack the small wins. It is small
*validated* changes, not small hopeful ones. *Use when* a goal is too big to solve in one move.
